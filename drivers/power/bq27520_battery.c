				 ...
				 const struct i2c_device_id *id)
{
	...
	{
		dev_err(&client->dev, "No support for I2C functionality\n");
		return -ENODEV;
	}

	di = kzalloc(sizeof(*di), GFP_KERNEL);
	if (!di) {
		dev_err(&client->dev, "Can't alloc device info data\n");
		return -ENOMEM;
	}


	if (dev->of_node)
		ret = bq27520_setup_dt(di);
	else
		ret = bq27520_setup_pdata(di, pdata);
...
#ifdef CONFIG_OF
static const struct of_device_id bq27520_of_match[] = {
	{ .compatible = "ti,bq27520", },
	{ },
};
MODULE_DEVICE_TABLE(of, bq24520_of_match);
#else
...
static struct i2c_driver bq27520_battery_driver = {
	.driver = {
		...
		.pm = &bq27520_pm_ops,
#endif
		.of_match_table = of_match_ptr(bq24520_of_match),
	},
